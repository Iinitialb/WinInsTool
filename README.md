<p align="center">
  <img width="480" height="320" alt="wininstool (Custom)" src="https://github.com/user-attachments/assets/f9f106e1-259b-40fc-ac44-c80c082bf252" />
</p>  
<table>
  <tr>
    <!-- Releases Button -->
    <td>
      <a href="https://github.com/Iinitialb/WinInsTool/releases" style="text-decoration:none;">
        <table style="border-collapse:collapse;border-radius:6px;overflow:hidden;font-family:sans-serif;">
          <tr>
            <td style="background-color:#2c2c2c;color:white;padding:10px 20px;font-weight:bold;border:none;">
              Releases
            </td>
          </tr>
        </table>
      </a>
    </td>
    <td style="width:10px;"></td>
    <!-- Wiki Button -->
    <td>
      <a href="https://github.com/Iinitialb/WinInsTool/wiki" style="text-decoration:none;">
        <table style="border-collapse:collapse;border-radius:6px;overflow:hidden;font-family:sans-serif;">
          <tr>
            <td style="background-color:#2c2c2c;color:white;padding:10px 20px;font-weight:bold;border:none;">
              Wiki
            </td>
          </tr>
        </table>
      </a>
    </td>
    <td style="width:10px;"></td>
    <!-- Author Button -->
    <td>
      <a href="https://github.com/Iinitialb" style="text-decoration:none;">
        <table style="border-collapse:collapse;border-radius:6px;overflow:hidden;font-family:sans-serif;">
          <tr>
            <td style="background-color:#2c2c2c;color:white;padding:10px 20px;font-weight:bold;border:none;">
              Author
            </td>
          </tr>
        </table>
      </a>
    </td>
    <td style="width:10px;"></td>
    <!-- Project Button -->
    <td>
      <a href="https://github.com/Iinitialb/WinInsTool" style="text-decoration:none;">
        <table style="border-collapse:collapse;border-radius:6px;overflow:hidden;font-family:sans-serif;">
          <tr>
            <td style="background-color:#2c2c2c;color:white;padding:10px 20px;font-weight:bold;border:none;">
              Project stable
            </td>
          </tr>
        </table>
      </a>
    </td>
    <td style="width:10px;"></td>
    <!-- Preview Branch Button -->
    <td>
      <a href="https://github.com/Iinitialb/WinInsTool/tree/preview" style="text-decoration:none;">
        <table style="border-collapse:collapse;border-radius:6px;overflow:hidden;font-family:sans-serif;">
          <tr>
            <td style="background-color:#2c2c2c;color:white;padding:10px 20px;font-weight:bold;border:none;">
              Preview project model
            </td>
          </tr>
        </table>
      </a>
    </td>
  </tr>
</table>

# What is WinInsTool?
## Goal
- WinInsTool is a python script that shows you a list of **212** app to download to your fresh windows machine. It could **install** various web browser, programming tools (IDEs, programming laguages...), media players...
## Requirements
- You need to Windows 10 version 1809 (build 17763) or later to be able to use WinInsTool, to check your windows virsion open **Command Prompt** and run: <pre>systeminfo | findstr /B /C:"OS Name" /C:"OS Version"</pre>
and you should see something like this:
<p align="center">
  <img width="1734" height="987" alt="image" src="https://github.com/user-attachments/assets/c333a132-b38e-4aae-a9dd-4d51b02865b2" />
</p>
*or* if you like **PowerShell** you can use:
<pre>winver</pre>
then you should see in a new window:
<p align="center">
  <img width="691" height="621" alt="Screenshot 2025-11-16 175405" src="https://github.com/user-attachments/assets/293d1147-6322-4396-ad05-5c355c183dc3" />
</p>

## How to use?
-We recommend running WinInsTool.exe thought the **cmd** by going to the directory you downloaded in it WinInsTool.exe, then run:
<pre>WinInsTool.exex</pre>
*if* you do everything correctly you should see:
<p align="center">
  <img width="1734" height="983" alt="image" src="https://github.com/user-attachments/assets/f0459340-701b-4fc9-a75b-b21cf5fb8b68" />
</p>
